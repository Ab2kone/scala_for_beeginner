...
object Empty extends MyList[Nothing] {
  ...
  def trail: MyList[Nothing] = throw new NoSuchElementException
...
class Cons[+A](h: A, t:MyList[A]) extends MyList[A] {
  ...
  [1,2,3].map(n * 2)
    ...
    = new Cons(2, new Cons(4, new Cons(6, new Cons(6, Empty.map(n *2)))
